...
class datahlp(object):
    ...
    def lambda_q3(self, x, q0, qf, lp, ln, lf, dsep):
        ...
        if np.any(mask_private):
            y[mask_private] = q0 * np.exp((x[mask_private] - dsep) / lp)
        
        if np.any(mask_sol):
            y[mask_sol] = (q0 - qf) * np.exp(-(x[mask_sol] - dsep) / ln) + qf * np.exp(-(x[mask_sol] - dsep) / lf)
...
class datahl2(object):
    ...
    def get_midplane_conversion(self, psino, rmido, wedge_n):
    ...
    def update_total_flux(self):
        """Update total flux calculations."""
        # Implementation would depend on specific requirements
        pass
    ...
    def get_divertor(self, outer=True, lower=True):
        """Get divertor indices for analysis."""
        # Simple placeholder - would need actual divertor detection logic
        n = len(self.r[0])
        if outer and lower:
            return 0, n//4
        elif outer and not lower:
            return n//4, n//2
        elif not outer and lower:
            return n//2, 3*n//4
        else:
            return 3*n//4, n
...
def load_heatdiag2(xgc_instance):
    ...
    try:
        ...
        xgc_instance.hl2.rs, xgc_instance.hl2.rmidsepmm = xgc_instance.hl2.get_midplane_conversion(psino, rmido, wedge_n)
